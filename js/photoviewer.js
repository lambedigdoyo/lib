  ...
  var movable = {
    ...
    movable: function movable(stage, image) {
      ...
      var dragStart = function dragStart(e) {
        ...
        startX = e.type === 'touchstart' ? e.originalEvent.targetTouches[0].pageX : e.clientX;
        startY = e.type === 'touchstart' ? e.originalEvent.targetTouches[0].pageY : e.clientY; // Î´ is the difference between image width and height

        Î´ = !self.isRotated ? 0 : (imageWidth - imageHeight) / 2; // Width or height difference can be use to limit image right or top position
        ...
        top = $(image).position().top + Î´; // Add grabbing cursor
    ...
    resizable: function resizable(modal, stage, image, minWidth, minHeight) {
      ...
      var getImageOpts = function getImageOpts(dir, offsetX, offsetY) {
        ...
        var widthDiff = offsetX + modalData.w > minWidth ? stageData.w - imgWidth + offsetX - Î´ : minWidth - (modalData.w - stageData.w) - imgWidth - Î´,
            ...
            heightDiff2 = -offsetY + modalData.h > minHeight ? stageData.h - imgHeight - offsetY + Î´ : minHeight - (modalData.h - stageData.h) - imgHeight + Î´; // Get image position in dragging

        var imgLeft = (widthDiff > 0 ? $(image).position().left : $(image).position().left < 0 ? $(image).position().left : 0) - Î´,
            imgTop = (heightDiff > 0 ? $(image).position().top : $(image).position().top < 0 ? $(image).position().top : 0) + Î´,
            imgLeft2 = (widthDiff2 > 0 ? $(image).position().left : $(image).position().left < 0 ? $(image).position().left : 0) - Î´,
            imgTop2 = (heightDiff2 > 0 ? $(image).position().top : $(image).position().top < 0 ? $(image).position().top : 0) + Î´;
        var opts = {
          'e': {
  ...
  function () {
    ...
    _proto.zoomTo = function zoomTo(ratio, origin, e) {
      ...
      newLeft = origin.x - (origin.x - imgData.x) / imgData.w * newWidth,
          newTop = origin.y - (origin.y - imgData.y) / imgData.h * newHeight; // Î´ is the difference between image new width and new height
